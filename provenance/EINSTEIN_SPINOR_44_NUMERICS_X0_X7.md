...
The canonical replay is byte-identical in both `history.csv` and
`summary.json`. The tighter run uses `rtol=1e-12`, `atol=1e-14`, and maximum
step `0.001`; it takes 2,294 solver steps and 2,422 right-hand-side
evaluations. Its maximum normalized state difference from the canonical run is

$$
6.310776406656671\times10^{-10}.
$$

Independent recomputation gives refined maximum relative errors

$$
(\varepsilon_S,\varepsilon_\rho,\varepsilon_F)
=(1.6415774988196702\times10^{-9},
3.2855796418252515\times10^{-10},
3.31034411227924\times10^{-10}),
$$

all smaller than their canonical counterparts.
...
The tracked `evidence.json` records exact component pairings, source hashes,
solver configuration, four exact model checks, 22 numerical checks, and
dark-sector scope. The tracked `convergence.json` records all 24
replay/refinement checks. The Phase 7 checker pins both files by SHA-256, and
the gate regenerates each file twice and requires byte identity.
