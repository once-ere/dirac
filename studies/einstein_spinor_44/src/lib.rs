...
pub use generated::{
    CL44_FIXTURE_SHA256, GEOMETRY_FIXTURE_SHA256, SPINOR_BILINEAR, SPINOR_DIMENSION,
    STATE_DIMENSION, TIME_GAMMA,
};
...
pub fn rhs_values(state: &State) -> Result<State, String> {
    ...
    derivative[1] = -KAPPA_8 / (TRANSVERSE_DIMENSION - 1.0) * (density + isotropic_pressure);
    for row in 0..SPINOR_DIMENSION {
        derivative[2 + row] = -0.5 * TRANSVERSE_DIMENSION * hubble * psi[row];
        for column in 0..SPINOR_DIMENSION {
...
fn sample(time: f64, state: State) -> Result<Sample, String> {
    ...
    let hubble_derivative =
        -KAPPA_8 / (TRANSVERSE_DIMENSION - 1.0) * (density + isotropic_pressure);
    let acceleration = hubble * hubble + hubble_derivative;
...
mod tests {
    ...
    #[test]
    fn present_state_satisfies_both_einstein_equations() {
        ...
        assert!((density - 1.0).abs() < 1.0e-15);
        assert!((isotropic_pressure + 0.76).abs() < 1.0e-15);
        assert!((state[1] * state[1] - density).abs() < 1.0e-15);
        let spatial =
            6.0 * derivative[1] + 21.0 * state[1] * state[1] + KAPPA_8 * isotropic_pressure;
        assert!(spatial.abs() < 1.0e-14);
        let d_condensate = 2.0
            * psi
                ...
                .sum::<f64>();
        assert!((d_condensate + 7.0).abs() < 1.0e-14);
    }
    ...
    fn rejects_nonpositive_geometry_or_condensate() {
        ...
        assert_eq!(spinor_bilinear(&spinor(&zero_condensate)), 0.0);
    ...
    #[test]
    fn cvode_solution_preserves_constraint_and_accelerates() {
        let result = integrate(&Config::default()).expect("CVODE integration");
        ...
        let transition = result
            ...
            .expect("deceleration-to-acceleration transition");
        assert_eq!(transition.1, acceleration_transition_scale_factor());
        assert!((transition.1 - 0.8631436165767084).abs() < 1.0e-15);
        assert!(result.samples.last().expect("last sample").acceleration > 0.0);
    }
}
